...
- 2026-05-23: `shogi-auto-cpu` の対局中停止画像から、赤ハイライト付き `w_+N` が `w_+S` に寄るケースを確認。`templates/pieces/w_+N/highlighted_promoted_knight_r7c3.png` を追加し、相互干渉で既存 `w_+S` fixture が崩れないよう `templates/pieces/w_+S/promoted_silver_highlight_balance_r8c3.png` も追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-23: `shogi-auto-cpu` の停止画像から、白持ち駒の桂2枚表示が `best=2, score=0.692` で数字しきい値を下回るケースを確認。`templates/hand_digits/2/white_knight_digit_2_live_w_N.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-23: `shogi-auto-cpu` の中盤停止画像から、赤ハイライト付き `b_+N` が `b_+S`、白 `w_+R` が `w_+B` に寄るケースを確認。`templates/pieces/b_+N/promoted_knight_rook_double_highlight_r3c6.png` と `templates/pieces/w_+R/promoted_knight_rook_double_highlight_r4c8.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-24: `shogi-auto-cpu` の停止画像から、盤端の赤ハイライト付き `w_+P` が `score=0.670` で盤上しきい値を下回るケースを確認。`templates/pieces/w_+P/white_promoted_pawn_edge_highlight_r7c9.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-24: `shogi-auto-cpu` の停止画像から、左下隅の赤ハイライト付き `w_+L` が `w_+S score=0.587` に寄るケースを確認。`templates/pieces/w_+L/white_promoted_lance_corner_highlight_r9c9.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-24: `shogi-auto-cpu` の停止画像から、白持ち駒の銀3枚表示が `best=3, score=0.642` で数字しきい値を下回るケースを確認。`templates/hand_digits/3/white_silver_digit_3_live_w_S.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-24: `shogi-auto-cpu` の停止画像から、黒持ち駒の金3枚表示が `best=2, score=0.629` に寄るケースを確認。`templates/hand_digits/3/black_gold_digit_3_live_b_G.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-24: `shogi-auto-cpu` の停止画像から、青ハイライトで選択された黒歩が空マス扱いになるケースを確認。`templates/pieces/b_P/selected_black_pawn_blue_r7c7.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
- 2026-05-24: `shogi-auto-cpu` の停止画像から、白持ち駒の桂4枚表示で桂スロットが `piece_score=0.603`、数字4が `best=2, score=0.645` になるケースを確認。`templates/pieces/w_N/white_knight_hand_4_live_w_N_hand.png` と `templates/hand_digits/4/white_knight_hand_4_live_w_N.png` を追加。root側 `python tools\recognition_fixtures.py check`、root側 `python -m unittest discover -s tests`、本repo側 `python -m unittest discover -s tests` がOK。
